...
export const addUserToProcedurePlan = async (userId, planId, procedureId) => {
    const url = `${api_url}/Plan/AddUserToProcedurePlan`;
...
export const getUserPlanProcedures = async (planId) => {
    const url = `${api_url}/PlanProcedure?$filter=planId eq ${planId}&$expand=procedure,UserPlanProcedures,UserPlanProcedures($expand=User)`;
    const response = await fetch(url, {
        method: "GET",
    });
